<template>
    <v-container >
        <v-toolbar flat theme="dark" density="compact" rounded color="primary" class="mb-3">
            ...
            <v-spacer></v-spacer>
            <v-btn variant="text"  theme="dark"  @click="toCheckout" :disabled="shoppingCart.length==0">Check Out</v-btn>
        </v-toolbar>
        <v-card outlined class=" ma-auto rounded-lg" v-for=" (item, index) in shoppingCart" :key="index" :max-width="viewPortDimension.width * 0.8">
            <v-card-text>
                <v-row>
                    <v-col cols="4">
                        <v-img :src="item.photo" max-height="200" max-width="200"></v-img>
                    </v-col>
                    <v-col cols="8">
                        <v-card-title class="text-subtitle-1 text--darken-3 text-fontColor">{{item.description}}</v-card-title>
                        ...
                        <v-row class="my-3 ml-3">
                            ...
                            <v-avatar tile size="30" class="rounded-lg mb-2"><v-img max-width="30" max-height="30" :src="item.vendorLogo"></v-img></v-avatar>
                        </v-row>
                        <v-row class="my-y ml-3">
                            ...
                            <v-select variant="outlined" 
                                ...
                                filled 
                                height="20"
                                @blur="updateShoppingCartQty(item.itemCode, item.quantity)" density="compact"></v-select>
                            </v-col>
                           <v-btn variant="text"  color="primary" class="mt-4" @click="removeShoppingCartItem(item.itemCode)">Remove</v-btn>
                        </v-row>
                    </v-col>
                </v-row>
            </v-card-text>
            ...
            <v-card-actions style="background-color: #445F9B;" >
                <v-spacer></v-spacer>
                <span class="mx-2 text-white font-weight-bold">Subtotal: {{ $filters.formatCurrencyAmount(item.promoRate>0?item.quantity*item.promoRate:item.quantity*item.rate) }}</span>
                <span class="mx-2 text-white font-weight-bold">Reward Silver: {{ $filters.formatIntAmount(item.rewardSilver*item.quantity) }}</span>
            </v-card-actions>
        </v-card>
         ...
         <v-card flat  class="pa-2" style="height: 100%; width: 100%;" v-if="shoppingCart.length==0">
            ...
            <v-row class="mt-3">
                <span class="text-h2 text-shade4 font-weight-bold mx-auto">Go Shopping ...</span>
            </v-row>
            </v-card-text>
        </v-card>
    </v-container>
